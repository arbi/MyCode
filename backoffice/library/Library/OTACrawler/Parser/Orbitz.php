...
class Orbitz extends Parser implements ParserInterface
{
    public function getSellingStatus()
    {
        try {
            ...
            preg_match_all(
                '/<div.*class="hotelPriceInfo avgNightlyHotelPriceInfo".*>(.*)\<\/div>/iU',
                $htmlData,
                $matches,
                PREG_SET_ORDER
            );
            ...
            if (count($matches) && isset($matches[0][1])) {
                if (strpos(strtolower(strip_tags($matches[0][1])), 'night') !== false) {
