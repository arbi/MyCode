...
class Asana {
     ...
     * @return string JSON or null
     */
    public function getUsers($optFields = false) {
        $fields = '';

        if ($optFields) {
            $fields = '/?opt_fields=' . $optFields;
        }

        return $this->askAsana($this->userUrl . $fields);
    }
    ...
    public function addAttachmentToTask($taskId, array $data = array()) {
        ...
        if (class_exists('CURLFile', false)) {
            $data['file'] = new \CURLFile($data['file'], $mimeType, $finalFilename);
        } else {
            $data['file'] = "@{$data['file']}";
    ...
    private function askAsana($url, $data = null, $method = ASANA_METHOD_GET) {
        ...
        try {
            ...
            if ($this->debug || $this->advDebug) {
                ...
                if ($this->responseCode['http_code'] == 0) {
