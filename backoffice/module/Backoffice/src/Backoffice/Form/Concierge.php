...
class Concierge extends FormBase {
    /**
    ...
    public function __construct($name, $data, $options, $global) {
        ...
        if (isset($accGroupManageMain)) {
            if (!isset($peopleListForGroupManagerId[$accGroupManageMain->getGroupManagerId()])) {
		...
		if (is_object($data)) {
			if($accGroupManageMain->getIsApartel()) {
				$isAppartel['checked'] = 'checked';
			}
		}
    ...
    {
        return (['' => '-- Timezone List --'] + Objects::getTimezoneOptions());
    }
}
